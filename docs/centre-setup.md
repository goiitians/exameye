...
4. Trigger the real screensaver or lock screen (hot corner, or wait out the machine's idle timeout) and then resume. Popup "Counters" must show `SCREENSAVER: 1`, and `log.txt` in the session folder must contain a line with `IDLE_START state=locked`. If the platform never reports a locked state, `IDLE_START state=idle` is expected instead - this is the documented `idle` fallback classification and is not a fault.
