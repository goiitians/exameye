...
test('shouldPrompt: off/stopped/error yes, prompting/on no', () => {
  const at = T0;
  assert.equal(shouldPrompt({ ...EMPTY_DESKTOP, state: 'off' }, { at, repromptSec: 300 }), true);
  assert.equal(shouldPrompt({ ...EMPTY_DESKTOP, state: 'stopped' }, { at, repromptSec: 300 }), true);
  assert.equal(shouldPrompt({ ...EMPTY_DESKTOP, state: 'error' }, { at, repromptSec: 300 }), true);
  assert.equal(shouldPrompt({ ...EMPTY_DESKTOP, state: 'prompting' }, { at, repromptSec: 300 }), false);
  assert.equal(shouldPrompt({ ...EMPTY_DESKTOP, state: 'on' }, { at, repromptSec: 300 }), false);
  assert.equal(shouldPrompt(undefined, { at, repromptSec: 300 }), true);
});

test('declined re-asks only after the interval and only when repromptSec > 0', () => {
  const declined = { ...EMPTY_DESKTOP, state: 'declined', at: T0 };
  assert.equal(shouldPrompt(declined, { at: T0 + 4 * 60000, repromptSec: 300 }), false);
  assert.equal(shouldPrompt(declined, { at: T0 + 5 * 60000, repromptSec: 300 }), true);
  assert.equal(shouldPrompt(declined, { at: T0 + 600000, repromptSec: 0 }), false);
});

test('started → on with MINIMIZE and alarm clear', () => {
  const prompting = { ...EMPTY_DESKTOP, state: 'prompting', at: T0, asks: 1 };
  const r = onHolder(prompting, { name: 'started', width: 1920, height: 1080, pickMs: 400 }, { at: T0 + 1000, repromptSec: 300 });
...
test('cancelled → declined with the alarm when repromptSec > 0', () => {
  const prompting = { ...EMPTY_DESKTOP, state: 'prompting', at: T0, asks: 2 };
  const r = onHolder(prompting, { name: 'cancelled', pickMs: 300 }, { at: T0 + 1000, repromptSec: 300 });
  ...
  const r2 = onHolder(prompting, { name: 'cancelled', pickMs: 300 }, { at: T0 + 1000, repromptSec: 0 });
...
test('failed → error', () => {
  const prompting = { ...EMPTY_DESKTOP, state: 'prompting', at: T0, asks: 1 };
  const r = onHolder(prompting, { name: 'failed', error: 'NotAllowedError' }, { at: T0 + 1000, repromptSec: 300 });
...
test('ended → stopped with REASK', () => {
  ...
  const r = onHolder(on, { name: 'ended' }, { at: T0 + 2000, repromptSec: 300 });
...
test('closed while on / while prompting / after SW close', () => {
  ...
  const r1 = onHolder(on, { name: 'closed' }, { at: T0 + 500, repromptSec: 300 });
  ...
  const r2 = onHolder(prompting, { name: 'closed' }, { at: T0 + 500, repromptSec: 300 });
  ...
  const r3 = onHolder(off, { name: 'closed' }, { at: T0 + 500, repromptSec: 300 });
...
test('dead only matters while on', () => {
  const on = { ...EMPTY_DESKTOP, state: 'on', at: T0, since: T0 };
  const r1 = onHolder(on, { name: 'dead', error: 'no answer' }, { at: T0 + 500, repromptSec: 300 });
  assert.equal(r1.desktop.state, 'stopped');
  assert.deepEqual(r1.input, { kind: 'DESKTOP', name: 'STOPPED', data: { reason: 'error', error: 'no answer' }, at: T0 + 500 });
  assert.deepEqual(r1.effects, [{ type: 'REASK' }]);

  const declined = { ...EMPTY_DESKTOP, state: 'declined', at: T0 };
  const r2 = onHolder(declined, { name: 'dead', error: 'no answer' }, { at: T0 + 500, repromptSec: 300 });
...
test('closed clears holderTabId as well as holderWindowId', () => {
  ...
  const r = onHolder(on, { name: 'closed' }, { at: T0 + 1, repromptSec: 300 });
...
test('onHolder does not mutate its input', () => {
  ...
  onHolder(on, { name: 'ended' }, { at: T0 + 1000, repromptSec: 300 });
  assert.deepEqual(on, before);
});

test('started copies screens into the desktop state and the input; EMPTY_DESKTOP.screens is null', () => {
  const r = onHolder(EMPTY_DESKTOP, { name: 'started', width: 1, height: 1, pickMs: 5, screens: 2 }, { at: T0, repromptSec: 300 });
  assert.equal(r.desktop.screens, 2);
  assert.equal(r.input.data.screens, 2);
  assert.equal(EMPTY_DESKTOP.screens, null);
  assert.equal(onHolder(EMPTY_DESKTOP, { name: 'started', width: 1, height: 1, pickMs: 5 }, { at: T0, repromptSec: 300 }).desktop.screens, null);
});
